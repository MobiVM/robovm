...
public final class NetworkInterface extends Object {
    ...
    NetworkInterface(String name, int interfaceIndex,
    ...
    NetworkInterface(String name, int interfaceIndex, InetAddress[] addresses) {
        this(name, interfaceIndex, Arrays.asList(addresses), Collections.<InterfaceAddress>emptyList());
    }
    ...
    private static void collectIpv6Addresses(String interfaceName, int interfaceIndex,
        ...
        if (bytes != null) {
            for (int i = 0; i < bytes.length; i += 32) {
                ...
                Inet6Address inet6Address = new Inet6Address(null, addressBytes, interfaceIndex);
                Inet6Address inet6Mask = new Inet6Address(null, netmaskBytes, interfaceIndex);
                addresses.add(inet6Address);
                interfaceAddresses.add(new InterfaceAddress(inet6Address, null, inet6Mask));
            }
        }
    }
        ...
        if (bytes != null) {
            for (int i = 0; i < bytes.length; i += 12) {
                Inet4Address address = new Inet4Address(
                        null, new byte[] { bytes[i + 0], bytes[i + 1], bytes[i + 2], bytes[i + 3] });
                Inet4Address netmask = new Inet4Address(
                        null, new byte[] { bytes[i + 4], bytes[i + 5], bytes[i + 6], bytes[i + 7] });
                Inet4Address broadcast = new Inet4Address(
                        null, new byte[] { bytes[i + 8], bytes[i + 9], bytes[i + 10], bytes[i + 11] });
