...
/**
 * @author Achrouf Abdenour The GlobalTrampolineCompiler act like the
 *         TrampolineCompiler class but have 2 differences. it will not find any
 *         class dependencies since the ClassPreprocessor has did it before.
 *         this class reley on devirtualisation results to create the trampoline
 *         alias for interface and virtual calls rather us.
 */

public class GlobalTrampolineCompiler {
    ...
    public GlobalTrampolineCompiler(Config config) {
        this.config = config;
    }
    ...
    public void compile(ModuleBuilder mb, Clazz currentClass, Trampoline t) {
        ...
        if (t instanceof New) {
            ...
            if (target.isAbstract() || target.isInterface()) {
                call(errorFn, BC_THROW_INSTANTIATION_ERROR, errorFn.getParameterRef(0),
                        mb.getString(t.getTarget().replace('/', '.')));
    ...
    private void createTrampolineAliasForMethod(Invoke t, SootMethod rm) {
        ...
        /*
         * We check first if this method is a devirtualized one If the returned
         * method is not null we call the direct method else we we create the
         * lookup function Note: this is applied only for virtual and interface
         * methods
         */
    ...
    private boolean checkMemberAccessible(Function f, Trampoline t, ClassMember member) {
        ...
        if (runtimeClassName != null && !isArray(runtimeClassName)) {
            ...
            if (c == null) {
                // The runtime class type is not available. Classloading will
                // fail earlier so let's
